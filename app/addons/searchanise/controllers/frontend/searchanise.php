...
if ($mode == 'async') {
    ...
    while (!empty($q)) {
        ...
        if ($q['error_count'] >= SE_MAX_ERROR_COUNT) {
            fn_se_set_import_status(SE_STATUS_SYNC_ERROR, $company_id, $lang_code);
            die('DISABLED');
        }
        ...
        if ($q['action'] == 'prepare_full_import') {
            ...
            if ($status == true) {
                fn_se_set_import_status(SE_STATUS_PROCESSING, $company_id, $lang_code);
            }
            ...
            if ($status == true) {
                fn_se_set_import_status(SE_STATUS_SENT, $company_id, $lang_code);

...
{if $href_delete}
    <a {if $id}id="rm_{$id}"{/if}
       class="delete-item cm-tooltip {if !$no_confirm}cm-confirm{/if}{if $microformats} {$microformats}{/if}"
       title="{__("remove")}"
...
{if !$href_delete && !$href_clone}
    <button type="button"
            class="btn-link btn-link--contents delete-item cm-tooltip {if !$no_confirm}cm-confirm{/if}{if $microformats} {$microformats}{/if}"
            title="{__("remove")}"
            {if $delete_target_id}data-ca-target-id="{$delete_target_id}"{/if}
    >
        <i class="icon-remove"></i>
